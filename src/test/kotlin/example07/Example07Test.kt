...
class Example07Test {
    private fun newSession(): SqlSession {
        ...
        config.addMapper(Example07Mapper::class.java)
    ...
    fun selectPersonWithAllFields() {
        newSession().use { session ->
            val mapper = session.getMapper(Example07Mapper::class.java)

            val person = mapper.selectPersonByIdWithAnnotations(1)
    ...
    fun selectPersonWithNullOccupation() {
        newSession().use { session ->
            ...
            val person = mapper.selectPersonByIdWithAnnotations(3)
    ...
    @Test
    fun selectPersonWithNullOccupationAndXMLMapping() {
        newSession().use { session ->
            val mapper = session.getMapper(Example07Mapper::class.java)

            val person = mapper.selectPersonByIdWithXMLMapping(3)
    ...
    @Test
    fun selectPersonWithAllFieldsAndXMLMapping() {
        newSession().use { session ->
            val mapper = session.getMapper(Example07Mapper::class.java)

            val person = mapper.selectPersonByIdWithXMLMapping(1)
